...
import { AddPatientModal } from './add-patien-modal';
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbLink,
...
export function SiteHeader() {
  ...
  const currentTabArabic = tabs.find((tab) => tab.currentTabName === currentTab)?.currentTabNameAr;

  return (
    <header className="bg-background sticky top-0 z-50 flex w-full items-center border-b">
      <div className="flex h-(--header-height) w-full items-center gap-2 px-4">
        <Button className="h-8 w-8 rotate-180" variant="ghost" size="icon" onClick={toggleSidebar}>
          <SidebarIcon />
        </Button>
      ...
      <AddPatientModal open={isAddPatientModalOpen} onOpenChange={setIsAddPatientModalOpen} />
    </header>
  );
}
